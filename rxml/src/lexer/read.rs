	...
	pub fn feed(&mut self, c: u8) -> Result<Option<Utf8Char>> {
		...
		if self.seqlen == 0 {
			...
			let (raw, len, required_cont_mask) = match c {
				...
				_ => return Err(Error::InvalidUtf8Byte(c)),
			};
			self.accum = raw;
			self.seqlen = len;
			self.buf = [c, 0, 0, 0];
			...
			if c & 0xc0 != 0x80 || c & self.cont_mask == 0 {
				self.reset();
				return Err(Error::InvalidUtf8Byte(c));
			}
			self.cont_mask = 0xff;
...
mod tests {
	...
	fn decoding_reader_rejects_invalid_start_byte() {
		...
		let mut r = DecodingReader::new(&mut chunked);
		let (v, err) = r.read_all();
		assert!(matches!(err, Err(Error::InvalidUtf8Byte(0xffu8))));
	...
	fn decoding_reader_fuzz() {
		...
		let (_, err) = r.read_all();
		match err {
			Err(Error::InvalidUtf8Byte(158)) => Ok(()),
			Err(e) => Err(e),
			Ok(()) => panic!("expected error"),
		}.unwrap(); // let this panic usefully on mis-match
	}
	...
	fn read_validated_passes_error() {
		...
		let mut out = String::new();
		let result = read_validated(&mut r, &AllChars(), 128, &mut out);
		assert!(matches!(result, Err(Error::InvalidUtf8Byte(0xff))));
